...
class TurbigenConfig:
    """Top level configuration class for turbigen_ref.
    ...
    fluid: turbigen_ref.fluid.FluidConfig
    """Equation of state."""

    mean_line: turbigen_ref.meanline_new.MeanLineConfig
    """Settings for the mean-line designer."""

    inlet: turbigen_ref.inlet.InletConfig = None
    ...
    mesh: turbigen_ref.mesh.Mesher = None
    """Settings for mesh generation."""

    solver: turbigen_ref.solvers.base.BaseSolver = None
    ...
    job: turbigen_ref.job.BaseJob = None
    """Settings for queue job submission."""

    converged: bool = False
    """Flag to indicate iterative convergence."""

    design_space: turbigen_ref.dspace.DesignSpace = None
    ...
    def save(self, fname=None, overwrite_pkl=True, use_gzip=True, write_grids=True):
        ...
        try:
            turbigen_ref.yaml_utils.write_yaml(data, conf_fname)
        ...
        for py_file in py_files:
            ...
            try:
                ...
                spec = importlib.util.spec_from_file_location(
                    f"turbigen_ref.plugin.{module_name}", py_file
                )
                module = importlib.util.module_from_spec(spec)
                sys.modules[f"turbigen_ref.plugin.{module_name}"] = module
    ...
    def __post_init__(self):
        ...
        self.fluid = turbigen_ref.fluid.FluidConfig.from_dict(self.fluid)
        ...
        if self.inlet:
            self.inlet = turbigen_ref.inlet.InletConfig(**self.inlet)

        # Set up the meanline designer
        self.mean_line = turbigen_ref.meanline_new.MeanLineConfig.from_dict(self.mean_line)

        if isinstance(self.mixed_out_flowfield, dict):
            self.mean_line.actual = turbigen_ref.meanline_data.meanline_from_dump(
        ...
        if self.annulus:
            AnnulusDesigner = util.get_subclass_by_name(
                turbigen_ref.annulus.AnnulusDesigner, self.annulus.pop("type", "smooth")
            )
            self.annulus = AnnulusDesigner(self.annulus)

        if self.operating_point:
            self.operating_point = turbigen_ref.op_point.OperatingPoint(
        ...
        for row in self.blades:
            ...
            blades.append([])
            for blade in row:
                blades[-1].append(turbigen_ref.blade.BladeDesigner(**blade))
        self.blades = blades

        # Convert nblade dict to NbladeConfig objects
        self.nblade = [
            util.init_subclass_by_signature(turbigen_ref.nblade.BladeNumberConfig, d)
        ...
        if self.mesh:
            ...
            if mesh_type == "h":
                Mesher = turbigen_ref.hmesh.H
            elif mesh_type == "oh":
                Mesher = turbigen_ref.ohmesh.OH
            self.mesh = Mesher(**self.mesh)

        # Lazy import the solver
        if self.solver:
            solver_name = self.solver.pop("type", "ember")
            importlib.import_module(f".{solver_name}", package="turbigen_ref.solvers")
            Solver = util.get_subclass_by_name(
                turbigen_ref.solvers.base.BaseSolver, solver_name
            )
            ...
            if isinstance(self.solver.convergence, str) and not self._fast_init:
                self.solver.convergence = turbigen_ref.solvers.base.ConvergenceHistory.load(
        ...
        if self.iterate:
            ...
            for k, v in self.iterate.items():
                ...
                cls = util.get_subclass_by_name(turbigen_ref.iterators.IteratorConfig, k)
                iter_cls.append(cls)
                if v:
            ...
            if turbigen_ref.iterators.Incidence in iter_cls:
                index = iter_cls.index(turbigen_ref.iterators.Incidence)
                self.iterate.insert(0, self.iterate.pop(index))
                assert self.iterate[0].__class__ == turbigen_ref.iterators.Incidence
        ...
        if self.post_process:
            ...
            for ip, p in enumerate(self.post_process):
                ...
                cls = util.get_subclass_by_name(turbigen_ref.post.BasePost, type)
                if p:
                    # Pass the dictionary to the subclass
                    posts.append(cls(**p))
                # Null content implies all defaults
        ...
        if j := self.job:
            ...
            cls = util.get_subclass_by_name(turbigen_ref.job.BaseJob, type)
            self.job = cls(**j)

        # Add some default post processors
        defaults = [
            turbigen_ref.post.SurfaceDistribution(),
            turbigen_ref.post.Convergence(),
            turbigen_ref.post.Annulus(),
            turbigen_ref.post.Metadata(),
        ]
        for d in defaults:
            found = False
        ...
        if self.design_space:
            if isinstance(self.design_space, dict):
                self.design_space = turbigen_ref.dspace.DesignSpace(**self.design_space)
    ...
    def get_machine(self):
        return turbigen_ref.geometry.Machine(
