...
class Blade:
    ...
    def _Thick(self):
        if self.thick_type:
            Thick = getattr(turbigen_ref.thickness, self.thick_type)
        else:
            Thick = turbigen_ref.thickness.Taylor
        return Thick

    @property
    def _Cam(self):
        if self.camber_type:
            Cam = turbigen_ref.camber.__dict__[self.camber_type]
        else:
            Cam = turbigen_ref.camber.Quartic
        return Cam
