...
def _write_blade(f, bld, nspan, nchord):
    ...
    nj = xyz.shape[2]
    vec_norm = turbigen_ref.util.fit_plane(xyz[:, :, nj // 2])
    basis1, basis2 = turbigen_ref.util.basis_from_normal(vec_norm)

    # Project the points onto the plane basis
    projected = turbigen_ref.util.project_onto_plane(xyz, basis1, basis2)

    # Verify that the signed area is same for all sections
    area = np.array(
        [turbigen_ref.util.shoelace_formula(projected[..., j]) for j in range(nj)]
    )
