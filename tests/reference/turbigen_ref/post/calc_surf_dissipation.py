...
def post(
    ...
    # dist = turbigen_ref.util.signed_distance(xr_cut, block.xr)
    ...
    def _integrate_cut(C, rel, w=None):
        ...
        rho = turbigen_ref.util.node_to_face(C.rho)
        T = turbigen_ref.util.node_to_face(C.T)
        x = turbigen_ref.util.node_to_face(C.x)
        r = turbigen_ref.util.node_to_face(C.r)
        xr = np.stack((x, r))

        # Isentropic to local static pressure
        Cs = C.copy().set_P_s(C.P, sin)
        hs = turbigen_ref.util.node_to_face(Cs.h)

        # Subtract isentropic static enthalpy from real relative stagnation
        # enthalpy to get relative isenstopic exit dynamic head
        if rel:
            ho_rel = turbigen_ref.util.node_to_face(C.ho_rel)
            Vs = np.sqrt(2.0 * np.maximum(ho_rel - hs, 0.0))
        else:
            ho = turbigen_ref.util.node_to_face(C.ho)
        ...
        if w is None:
            ...
            wf = turbigen_ref.util.node_to_face(w.squeeze())

        # Exclude cells downstream of last cut plane
        dist = turbigen_ref.util.signed_distance(xr_cut, xr)
        wf[dist > 0.0] = 0.0
    ...
    for irow, row_block in enumerate(grid.row_blocks):
        for block in row_block:
            ...
            for patch in block.patches:
                ...
                if isinstance(patch, turbigen_ref.grid.RotatingPatch):
                    is_rot[patch.get_slice()] = 1.0
                if type(patch) in turbigen_ref.grid.NOT_WALL_PATCHES:
