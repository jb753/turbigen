...
def post(
    ...
    Csc._data[:] = turbigen_ref.util.node_to_face(Cs._data)

    dAi = Cs.dAi
    mflux = Csc.rho * Csc.Vxrt
    mflow = turbigen_ref.util.dot(mflux, dAi).sum(axis=-1).squeeze()
    dAmag = turbigen_ref.util.vecnorm(dAi)
    area = dAmag.sum(axis=-1).squeeze()
    spf = Csc.spf[0, :, 0]

    def mass_avg(var):
        numer = turbigen_ref.util.dot(mflux * var, dAi).sum(axis=-1).squeeze()
        return numer / mflow
