...
class ts4(BaseSolver):
    ...
    :meth:`turbigen_ref.tables.make_tables` function following the example script below:

    .. code-block:: python

        from turbigen_ref.tables import make_tables
...
def run(grid, ts4_conf, machine, workdir):
    ...
    ofp["fluid_model"] = 0 if isinstance(grid[0], turbigen_ref.grid.PerfectBlock) else 1
...
""")
    ...
    ts3_conf = turbigen_ref.solvers.ts3.ts3(dts=0, workdir=ts4_conf.workdir).robust()
    ...
    if ts4_conf.nstep_ts3:
        ...
        turbigen_ref.solvers.ts3._run(grid, ts3_conf)
        grid.update_outlet()

    turbigen_ref.solvers.ts3._write_hdf5(grid, ts3_conf)
...
""") from None
    ...
    turbigen_ref.solvers.ts3._write_hdf5(grid, ts3_conf, fname="output_ts3.hdf5")
...
def read_probe_flow(fname, state):
    ...
    F = turbigen_ref.flowfield.PerfectFlowField(Omega.shape)
    F.Tu0 = 0.0
    F.xrt = xrt
    F.Vxrt = Vxrt
    F.cp = state.cp
