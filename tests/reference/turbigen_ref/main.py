...
def _make_run_parser():
    ...
    parser.add_argument(
        ...
        action="version",
        version=f"%(prog)s {turbigen_ref.__version__}",
    )
    parser.add_argument(
...
def cmd_run(args):
    ...
    d = turbigen_ref.yaml_utils.read_yaml(args.CONFIG_YAML)
    ...
    turbigen_ref.viewer.record_metadata(conf)
...
def cmd_sample(args):
    ...
    conf_dict = turbigen_ref.yaml_utils.read_yaml(config_path)
    ...
    if args.purge:
        from turbigen_ref.job_server import signal_daemon
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "sample":
        ...
        from turbigen_ref.job_server import _make_queue_parser, cmd_queue
