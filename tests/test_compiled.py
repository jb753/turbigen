...
def get_scale_fact(x):
    ...
    shape = x.shape
    return to_fort(np.ones(shape[:-1] + (3,))/3.)
...
def test_smooth_zero():
    ...
    Xs = to_fort(X)
    ssf = get_scale_fact(Xs)
    embsolve.smooth(Xs, ssf, sf2=0., sf4=0.)
...
def test_smooth_const():
    ...
    for sf2 in (0.1, 0.2):
        for sf4 in (0.1, 0.2):
            ...
            ssf = get_scale_fact(X)
            embsolve.smooth(X, ssf, sf2=sf2, sf4=sf4)
...
def test_smooth_linear():
    ...
    ssf = get_scale_fact(fs)
    embsolve.smooth(fs, ssf, sf2=0.1, sf4=0.)
    err_abs = np.abs(fs - f)
...
def test_smooth_cubic():
    ...
    fs = np.asfortranarray(f.copy())
    ssf = get_scale_fact(fs)
    embsolve.smooth(fs, ssf, sf2=0.0, sf4=0.1)
    err_abs = np.abs(fs - f)
    err_rel = err_abs/f.mean()
    assert np.allclose(f, fs)
...
def test_smooth_converge():
    ...
    for istep in range(10000):
        ...
        embsolve.smooth(Xnew, ssf, sf2, sf4)
        derr = X.ptp() - Xnew.ptp()
        X = Xnew
