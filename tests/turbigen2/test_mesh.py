...
from turbigen2 import H, Mesher, WallSpacing
...
def old_grid(machine, mesh, spacing):
    ...
    annulus = turbigen.annulus.MergedFixedAxialChord(
    ...
    mesher = turbigen.hmesh.H(**{k: v for k, v in mesh.items() if k != "type"})
    ...
    # that turbigen2's mesher performs, and the two would differ by an epsilon
    # that has nothing to do with the mesh mathematics being compared.
    reference.set_L_ref(H(**{k: v for k, v in mesh.items() if k != "type"}).L_ref(machine))
...
def test_matches_the_turbigen_implementation(machine, grid):
    reference = old_grid(machine, MESH, H(**{k: v for k, v in MESH.items() if k != "type"}).wall_spacing(machine))
