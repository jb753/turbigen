...
class OldRow:
    """The old package's mean-line interface, over a turbigen2 mean line.
...
def old_blade(machine, i_row, dchi_LE=-8.0, dchi_TE=0.0):
    ...
    designer = turbigen.blade.BladeDesigner(
        spf=np.array(SPF),
        camber=np.tile([dchi_LE, dchi_TE, CAMBER["aft_loading"]], (len(SPF), 1)),
        thick=np.tile(
            [THICKNESS[key] for key in ("R_LE", "t_max", "m_tmax", "kappa_max", "t_TE", "tanwedge")],
            (len(SPF), 1),
        ),
...
def test_circulation_count_matches_the_turbigen_implementation(machine):
    for i_row in range(2):
        ...
        expected = turbigen.nblade.Co(Co=0.6).get_blade_number(
