...
from turbigen2 import Config, Database, Result, case, database, iterate
...
def test_declared_variables_override_the_deduction(runs):
    config = query(1.5)
    config = dataclasses.replace(
        config, database=dataclasses.replace(config.database, variables=("mean_line.Ma2",))
    )
    samples = config.database.load(config, runs)
