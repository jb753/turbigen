...
and assembles them into a :class:`~turbigen2.meanline.MeanLine` --- the *actual*
mean line, against which the nominal one can be compared.

A free function of a grid and the machine it was meshed from, like
:mod:`turbigen2.guess` and :mod:`turbigen2.bconds`, so it can be run on a
...
def cut_planes(annulus, offset=CUT_OFFSET):
    ...
    :meth:`~turbigen2.annulus.Annulus.evaluate_xr`.
