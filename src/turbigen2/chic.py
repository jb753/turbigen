...
:attr:`~turbigen2.bconds.OperatingPoint.DP_adjust` moves a fixed machine off
...
:func:`turbigen2.iterate.converge` is a root find --- the two look alike and
...
:func:`turbigen2.iterate.converge` takes it, so the march can be driven by an
...
def at(config, DP_adjust):
    ...
    """
    from turbigen2.bconds import OperatingPoint  # noqa: PLC0415 - avoids a cycle
...
def sweep(config, run):
    ...
    run : callable
        Takes ``(config, index)`` and returns a :class:`~turbigen2.result.
