...
Runs of many cases, the design loop and a characteristic sweep, are
:mod:`turbigen.loop`.
...
def _iterate_one(args, config_path):
    ...
    with logging_into(args, config_path) as out_dir:
        ...
        _, result, converged, _ = loop.converge_design(
...
def _chic_one(args, config_path):
    ...
    with logging_into(args, config_path) as out_dir:
        ...
        settled = loop.design_is_settled(config_path, config)
        points, _ = loop.characteristic(config, out_dir, previous, settled)
...
def main(argv=None):
    ...
    try:
        ...
        # Always log the traceback. User input errors and genuine bugs both
        # arrive as ValueError, so there is no safe way to show only a summary
        # for the first kind.
