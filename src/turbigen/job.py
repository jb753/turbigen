...
class Job(Node):
    ...
    command: str = ""
    """The turbigen executable a submitted job should run. Empty means the one
    ...
    def executable(self):
        """Return the turbigen command a submitted job should run."""
        if self.command:
            return self.command
...
class Slurm(Job):
    ...
    *lines of a file* rather than directory names. The package this replaces
    ...
    range --- which the batches `turbigen batch` writes are not, since a point
    ...
    def directives(self, name):
        ...
        lines = [f"#SBATCH --job-name=turbigen_{name}"]
