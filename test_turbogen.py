    ...
    for phii in phi:
        for psii in psi:
            for Al1i in Al_range:
                    ...
                    psi_out = 2.*(1.-stg.Lam-phii*np.tan(np.radians(Al1i)))
    ...
    for phii in phi:
        for psii in psi:
            for etai in [0.8, 0.9, 1.0]:
                ...
                # Check efficiency
                eta_out = np.log(stg.To_Toin[-1])/np.log(stg.Po_Poin[-1])*ga/(ga-1.)
                ...
                Yp_stator_out = (1.-Po2_Po1)/(1.-1./Po1_P1)
                assert np.isclose(Yp_stator_out,stg.Yp[0])
                Yp_rotor_out = (1.-Po3_Po2_rel)/(1.-1./Po2_P2_rel)
    ...
    for phii in phi:
        for psii in psi:
            for Lami in Lam:
                ...
                psi_out = (1.-stg.To_Toin[2])/stg.U_sqrt_cpToin**2.
                assert np.isclose(psii,psi_out)
