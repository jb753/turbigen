...
class FanDiffuser:
    def __init__(
        self, rmid, span, Beta, AR, Lbody_rhub, Lbcond_span, htrmin, nozzle_ratio
    ):
        ...
        # Casing line
        rcas = rmid + span / 2.0
        rcas = np.insert(rcas, 0, np.full(3, rcas[0]))
        rcas = np.append(rcas, np.full(2, rcas[-1]))

        # Hub line
        rhub = rmid - span / 2.0
        rmin = rcas[0] * htrmin
        rhub = np.insert(rhub, 0, np.full(2, rmin))
        rhub = np.append(rhub, np.full(3, rmin))

        Lbody = np.array(Lbody_rhub) * rhub[2]
        Lbcond = Lbcond_span * (rcas - rhub)[0]

        # Aspect ratios set x within the compressor
        span_row = 0.5 * (span[1:] + span[:-1])
        x = turbigen.util.cumsum0(span_row / AR)

        # Upstream
        x = np.insert(x, 0, (-(Lbcond + Lbody[0]), -Lbody[0]))
        # Downstream
        x = np.append(
            x,
            (
                x[-1] + Lbody[1],
                x[-1] + Lbody[1] + Lbcond,
                x[-1] + Lbody[1] + Lbcond * 2,
            ),
        )

        dr_noz = (rcas - rhub)[-1] * (1 - nozzle_ratio) / 2.0

        rhub[-1] += dr_noz
        rcas[-1] -= dr_noz

        # Parameter vector: 0 is inlet, 1 rotor LE, 2 rotor TE, 3 stator LE ...
        Lup = x[:3].ptp()
        Ldn = x[-4:].ptp()
        dtup = np.array((0.0, Lbcond / Lup))
        dtdn = np.array((Lbody[1] / Ldn, (Lbody[1] + Lbcond) / Ldn, 1.0))
        t = np.concatenate((dtup, np.arange(1, len(span) + 1), len(span) + dtdn))

        self.hub = LinearLine(x, rhub, t)
        self.cas = LinearLine(x, rcas, t)
