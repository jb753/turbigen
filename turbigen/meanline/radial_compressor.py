...
def forward(
    ...
    #
    # (i) set rotor inlet state using, Marel1, Al1, phi, Ypigv
    #

    # Solve for the axial Mach at rotor inlet
    Max1 = Marel1 * (1.0 + (tanAlpha1 - 1.0 / phi1) ** 2.0) ** -0.5
    ...
    # Velocities
    Vrel2 = DHimp * Vrel1
    # Vr2 = Vrel2 / np.sqrt(1.0 + tanAlpharel2**2.0)
    Vr2 = Vrel2 * np.cos(np.radians(Alpharel2))
    # print(Vr2, Vr2_check)
    Vtrel2 = Vr2 * tanAlpharel2
...
def inverse(ml):
    ...
    out = {
        ...
        "Marel1": ml.Ma_rel[0],
        "htr1": ml.htr[0],
        "Alpharel2": ml.Alpha_rel[1],
        "DHimp": ml.V_rel[1] / ml.V_rel[0],
        "DHdiff": ml.V[-1] / ml.V[1],
