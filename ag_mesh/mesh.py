...
def run_remote(geomturbo, py_scripts, sh_script, gbcs_output_dir):
    ...
    for si in files:
        retcode = os.system('scp %s gp-111:%s' % ( si, tmpdir))
        if retcode:
            raise Exception('Copying to remote failed')

    # Run the shell script
    result = os.popen("ssh gp-111 'cd %s ; ./%s'" % (tmpdir, sh_script)).read()
    print(result)
...
if __name__ == '__main__':
    ...
    run_remote( '/mesh.geomTurbo', ['script_ag.py2', 'script_igg.py2'], 'script_sh', 'outdir')
