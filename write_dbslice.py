...
if __name__=="__main__":
    ...
    rpm = np.array([g.get_bv('rpm',bid) for bid in g.get_block_ids()])  # RPM in rotor row
    ...
    for bid in g.get_block_ids():
        ...
        for pid in g.get_patch_ids(bid):
            ...
            if patch.kind == ts_tstream_patch_kind.probe:
                ...
                fname_now = output_hdf5.replace('.hdf5','_probe_%d_%d.dat' % (bid,pid))
                ...
                vars_all.append(np.stack(
                    [np.squeeze(dat_now[ki].reshape((-1,dat_now[ki].shape[-1]))).transpose()
                        for ki in dat_now],axis=-1))
